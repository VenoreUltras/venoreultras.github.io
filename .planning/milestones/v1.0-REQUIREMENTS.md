# Requirements Archive: PM-300 Trener — Milestone v1.0

**Defined:** 2026-05-05
**Shipped:** 2026-05-28
**Status:** ✅ 64/64 requirements satisfied (audit: `.planning/v1.0-MILESTONE-AUDIT.md`)
...
> **Note:** Wszystkie wymagania `[x]` oznaczone jako ukończone na zamknięciu milestone v1.0. Bookkeeping checkboxes w trakcie pracy nie były synchronizowane na bieżąco — verification opiera się na 3-source cross-reference (REQUIREMENTS.md traceability + per-phase VERIFICATION.md + SUMMARY.md `requirements-completed` frontmatter) opisanej w MILESTONE-AUDIT.
...
- [x] **INFRA-02**: `tests/boundaries.test.js` statycznie wymusza granice importów (PressModel/Physics/Scene bez DOM, ProcedureEngine bez THREE/store/gsap, UI bez THREE)
...
- [x] **INFRA-05**: Obsługa utraty kontekstu WebGL — pauza tickera GSAP na `webglcontextlost`, polski komunikat overlay, auto-restore na `webglcontextrestored`

### State Management (STATE)

- [x] **STATE-01**: Zustand vanilla store `TrainingStore` jest jedynym źródłem prawdy dla stanu szkolenia (scenariusz, kroki, stan maszyny, stany meshy, eventy, scoring)
- [x] **STATE-02**: `mesh.userData` zawiera wyłącznie tożsamość (`id`, `kind`, `restPosition`, `labelPL`, `descriptionPL`) — nigdy status; egzekwowane przez code review checklist
- [x] **STATE-03**: Każdy subscriber store'a zwraca handle unsubscribe; `Application.dispose()` zwalnia wszystkie subskrypcje (Vite HMR `import.meta.hot?.dispose`)

### SOP Procedure Engine (SOP)

- [x] **SOP-01**: `ProcedureEngine` to czysta funkcja — `validateStep(intent, state, scenario)` zwraca `{ok, reason, effects[]}` bez efektów ubocznych, bez importów THREE/DOM/store
...
- [x] **SOP-03**: Scenariusz **uruchomienie** zaimplementowany — minimum 6 kroków obejmujących inspekcję wzrokową, sprawdzenie smarowania, zamknięcie osłon, odblokowanie E-stop, włączenie zasilania, sprzęgnięcie po nabraniu obrotów koła zamachowego
...
- [x] **SOP-05**: Scenariusz **zatrzymanie** zaimplementowany — kolejność: rozsprzęgnięcie, hamulec, wyłącznik główny, blokada
- [x] **SOP-06**: Scenariusz **awaria** zaimplementowany — przynajmniej 3 zdarzenia awaryjne (np. otwarcie osłony w cyklu, brak ciśnienia oleju, awaryjne zatrzymanie) z poprawną reakcją
...
- [x] **SOP-08**: Twardy gating — błędna akcja wyzwala WIDOCZNĄ porażkę (czerwony pulse, polski komunikat błędu, kara w scoringu), nigdy cichy skip
- [x] **SOP-09**: Wyczerpujące testy jednostkowe Vitest pokrywają wszystkie 4 scenariusze, wszystkie ścieżki sukces/porażka, edge case'y kolejności

### 3D Digital Twin (TWIN)

- [x] **TWIN-01**: Geometria PM-300 rozszerzona o **koło zamachowe** jako osobny clickable mesh
- [x] **TWIN-02**: Geometria PM-300 rozszerzona o **dźwignia sprzęgła** (clickable, animowana między pozycjami)
- [x] **TWIN-03**: Geometria PM-300 rozszerzona o **hamulec** (clickable, wizualny stan zaciągnięty/zwolniony)
- [x] **TWIN-04**: Geometria PM-300 rozszerzona o **wziernik smarowania / wskaźnik oleju** (clickable, kolor poziomu oleju)
- [x] **TWIN-05**: Geometria PM-300 rozszerzona o **osłona przednia ruchoma** (clickable, otwiera/zamyka)
- [x] **TWIN-06**: Geometria PM-300 rozszerzona o **osłona tylna stała** + **kolumny kurtyny świetlnej**
- [x] **TWIN-07**: Geometria PM-300 rozszerzona o **panel sterowania oburęczny** — 2 zielone przyciski startu, lampka gotowości
- [x] **TWIN-08**: Geometria PM-300 rozszerzona o **przycisk E-stop** (czerwony grzyb, clickable)
- [x] **TWIN-09**: Geometria PM-300 rozszerzona o **wyłącznik główny** (clickable, animowane przekręcenie)
- [x] **TWIN-10**: Geometria PM-300 rozszerzona o **tabliczka znamionowa** PM-300
- [x] **TWIN-11**: Każdy interactable mesh ma sklonowany własny `MeshStandardMaterial` (zapobiega CRIT-6 — wszystko świeci)
- [x] **TWIN-12**: `PressModel.getInteractables()` i `PressModel.getMeshDictionary()` zwracają rejestr klikalnych części
- [x] **TWIN-13**: Każdy interactable ma `userData = { id, kind, restPosition, labelPL, descriptionPL }`

### Interaction (INTERACT)

- [x] **INTERACT-01**: `RaycastController` — pojedynczy `Raycaster`, działa wyłącznie na `pointermove`/`pointerdown`, throttled do jednego raycastu per tick (NIE każda klatka)
- [x] **INTERACT-02**: Klik komponentu 3D emituje intencję (`{kind, meshId}`) → `store.attemptStep` → `ProcedureEngine.validateStep`
- [x] **INTERACT-03**: Hover komponentu 3D wyzwala wizualny hint (jasne podświetlenie) + tooltip
- [x] **INTERACT-04**: Hybrydowa interakcja — kroki typu `manipulation` wymagają kliknięcia w 3D (E-stop, osłony, dźwignia sprzęgła, wyłącznik); kroki typu `visual` wymagają zaznaczenia checkboxa w panelu (np. "sprawdziłem poziom oleju")
- [x] **INTERACT-05**: Walidator synchroniczny + lock `isAnimating` — double-click nie rejestruje dwóch poprawnych kroków (CRIT-8)
- [x] **INTERACT-06**: Skróty klawiszowe — `R` reset, `T` tryb wolny, `1-4` wybór scenariusza, `Space` start/pauza, `Esc` E-stop, `H` pomoc, `L` toggle etykiet 3D, `M` mute audio
...
- [x] **FEEDBACK-06**: Etykiety części 3D przez `CSS2DRenderer` — toggleable klawiszem `L`, polskie nazwy z `userData.labelPL`
...
- [x] **UI-03**: TooltipManager używa `@floating-ui/dom` z 600ms hover delay, pozycjonowanie auto-update przy ruchu
- [x] **UI-04**: Każdy aktywny krok wyświetla pole `rationale` po polsku ("po co ten krok") — pod krokiem lub za przyciskiem `?`
- [x] **UI-05**: Banner disclaimera widoczny stale — informuje że symulator NIE zastępuje rzeczywistego szkolenia BHP (CRIT-1)
...
- [x] **EDU-01**: Tryb wolny (free roam) — kursant może oglądać i klikać komponenty bez aktywnej procedury, eksplorując maszynę
- [x] **EDU-02**: Tryby trudności — **Nauka** (z hintami, podświetleniami "co dalej") i **Egzamin** (bez podpowiedzi, bez retry, finalny score)
- [x] **EDU-03**: Sygnały audio przez WebAudio — alarm awarii, potwierdzenie sukcesu, szum koła zamachowego (proporcjonalny do RPM), globalny mute (klawisz `M`)
- [x] **EDU-04**: Replay/timeline — kursant może odtworzyć przebieg sesji ze scrubbable timeline + slow-mo 0.25×
...
- [x] **SCORE-01**: `ScoringService` to czysta funkcja — kalkuluje wynik na bazie eventów ze store'a (-25 critical / -10 medium / -2 minor; finalne wagi do walidacji eksperta)
- [x] **SCORE-02**: Metryki sesji — liczba błędów, czas ukończenia, lista pominiętych kroków, lista naruszeń kolejności, liczba retry
- [x] **SCORE-03**: Persystencja w `localStorage` z wersjonowanym schema key `pm300:session:v1` — migracja graceful przy zmianie schematu
- [x] **SCORE-04**: Eksport JSON — pełny dump sesji (eventy, scoring, metadane uczestnika opcjonalnie)
...
- [x] **TEST-01**: Pokrycie testami jednostkowymi `ProcedureEngine` ≥ 95% — wszystkie scenariusze, wszystkie ścieżki walidacji, wszystkie fault rules
- [x] **TEST-02**: Testy `ScoringService` — czyste funkcje, edge cases, finalne wagi
- [x] **TEST-03**: Test boundaries (`tests/boundaries.test.js`) — statyczna asercja granic importów
- [x] **TEST-04**: Stress test 100 kliknięć w E-stop — brak race condition, scoring deterministyczny
- [x] **TEST-05**: Test integracyjny dla każdego z 4 scenariuszy — happy path + przynajmniej 2 ścieżki porażki
