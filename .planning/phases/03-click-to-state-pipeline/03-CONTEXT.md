...
- **D-Phase3-03: Intent shape mapuje na ProcedureEngine kontrakt z Phase 1 (NIE 1:1 z userData.kind).** `ProcedureEngine.validateStep` (Phase 1, locked) sprawdza `intent.kind === 'click' && intent.meshId === expectedStep.targetMeshId` dla kroków `manipulation` i `visual-target`, oraz `intent.kind === 'check' && intent.stepId === step.id` dla `visual-attest`. Dlatego:
  - **Manipulation/visual-target click** (RaycastController → pointerdown na mesh): `store.attemptStep({kind: 'click', meshId: userData.id})`. RaycastController odczytuje `userData.id` z hit, ale `intent.kind` jest **literałem `'click'`** (NIE `userData.kind`).
  - **Visual-attest button** (UI checkbox z panelu): `store.attemptStep({kind: 'check', stepId: currentStepId})`.
  - ProcedureEngine pozostaje pure bez zmian. RaycastController jest tępą rurą która translatuje pointer event → engine-compatible intent shape.
  - **Update z 2026-05-06:** wcześniejsza wersja D-Phase3-03 mówiła `{kind: userData.kind, meshId: userData.id}` (czyli `'manipulation'` / `'visual-attest'`) — to było niezgodne z faktycznym ProcedureEngine z Phase 1. Korekta po analizie Pitfall #2 z RESEARCH.md i bezpośrednim sprawdzeniu `src/training/ProcedureEngine.js` linie 46-47.
...
- **Visual-attest checkbox UI:** prosty `<button class="phase3-attest-check">Potwierdź: {labelPL}</button>`. Aria: `aria-label="Potwierdź krok: {labelPL}"`. Po kliku: `attemptStep({kind: 'check', stepId: currentStepId})` (zgodne z ProcedureEngine Phase 1). Re-render po `step.done`. Phase 4 zastąpi pełnym StepPanel z wszystkimi krokami.
