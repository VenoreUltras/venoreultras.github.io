...
- ✅ v1.2 — Rozbudowa edukacyjna i realizm (2026-06-19) — menu startowe z wyborem trybu, pełnoekranowy overlay (ElementInfoOverlay) zamiast bocznego panelu, instrukcje BHP + media w public/media/, płytka znamionowa (TextureLoader), egzamin hybrydowy (interakcja 3D + quiz BHP). 6 faz (12-17), 1010 testów, bundle 834.98 KB / 850 KB. Audit: `.planning/milestones/v1.2-MILESTONE-AUDIT.md`.
- ✅ v1.3 — Uproszczenie i dopracowanie egzaminu (2026-06-28) — usunięcie eksportu PDF/JSON (+ `jspdf`/`html2canvas`), panelu „Parametry Układu" i dźwięku HUM (alarm/confirm zostają); łączny wynik egzaminu (SOP+BHP proporcjonalnie, % + werdykt 80% + rozbicie) liczony w widoku; dostępny feedback odpowiedzi quizu (zielony/czerwony + ikona ✓/✗ + aria, blokada); responsywny modal quizu. Fixy UAT: modal quizu ukryty do otwarcia, menu trybu na każdym starcie. 3 fazy (18-20), 990 testów, bundle 827.96 KB < 834.98 KB. Audit: `.planning/milestones/v1.3-MILESTONE-AUDIT.md`.

## Current Milestone

_Brak aktywnego milestone._ v1.3 dostarczony 2026-06-28. Uruchom `/gsd:new-milestone`, aby rozpocząć kolejny cykl.

**Kandydaci na następny milestone (v2 docking points):** DIFF-01..04 — ExplodedViewController, randomized faults, supervisor recommendations, font scaling + high-contrast theme; adaptacyjny dobór pytań quizu wg błędów SOP (DIFF-05); video instruktażowe (VID-01/02).
...
- ✓ Kinematyka slider-crank: `y = r·cos(α) + √(l² − (r·sin(α))²)` — `src/PhysicsEngine.js` (obliczana w pętli; panel telemetrii „Parametry Układu" usunięty w v1.3)
- ✓ Panel boczny w glassmorphism (slider RPM, przycisk Start/Stop) — `src/UI.js`, `style.css`
- ✓ Polski język interfejsu i komentarzy
- ✓ Menu startowe z wyborem trybu (swobodny/nauka/egzamin) — pokazywane przy każdym starcie — v1.2/v1.3 — `src/ui/StartMenuOverlay.js`
- ✓ Egzamin hybrydowy: interakcja 3D (SOP) + quiz BHP z dostępnym feedbackiem (kolor+ikona+aria) i jednym łącznym wynikiem (% + werdykt 80% + rozbicie), liczonym w widoku z izolacją store — v1.2/v1.3 — `src/ui/QuizController.js`, `src/ui/SessionOverlay.js`
- ✓ Dźwięk edukacyjny: alarm (awaria) + confirm (krok) — `src/education/AudioController.js` (HUM silnika usunięty w v1.3)

### Active

_None._ v1.3 dostarczony i zarchiwizowany (`milestones/v1.3-REQUIREMENTS.md`). Nowy milestone definiuje się przez `/gsd:new-milestone`.

### Out of Scope

- **Eksport wyników (PDF/JSON)** — dostarczony w v1.0, **wycofany w v1.3** na życzenie; wynik egzaminu prezentowany wyłącznie na ekranie
- **Backend / konta użytkowników** — odbiorca to szkolenie wewnętrzne firmy, dane sesji trzymane lokalnie
...
---
*Last updated: 2026-06-28 — milestone v1.3 (Uproszczenie i dopracowanie egzaminu) ukończony i zarchiwizowany*
